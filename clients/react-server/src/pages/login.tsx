      ...
      <Button
        onClick={async () => {
          try {
            ...
            console.error('[Login] Exception during Google OAuth:', err);
          }
        }}
...
const LoginPasswordStep = ({
  ...
  const handleSubmit = async (data: { password: string }): Promise<void> => {
    ...
    try {
      const { error } = await signInWithEmail(email, data.password);
      if (error) throw error;
...
export const Login = (): JSX.Element => {
  ...
  const handleEmailNext = async (email: string): Promise<void> => {
    // Skip email check - just proceed to password step
    // User will see error if email doesn't exist when they try to login
